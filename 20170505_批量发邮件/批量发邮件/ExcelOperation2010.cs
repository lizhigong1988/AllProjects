...
namespace 批量发邮件
{
    public class ExcelOperation
    {
        ...
        /// <param name="dt"></param>
        public static System.Data.DataTable ImportSimpleExcel(string inputFile, List<string> listSheetNames = null)
        {
            ...
            else
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (listSheetNames.Contains(sheetName.Remove(sheetName.Length - 1)))
                    {
                        var strExcel = "select * from   [" + row["TABLE_NAME"].ToString() + "]"; ;
                        var myCommand = new OleDbDataAdapter(strExcel, strConn);
                        myCommand.Fill(ds);
